...
import { X, Download, Copy, Check, Film, MessageSquare } from 'lucide-react';
      ...
      <div className="bg-[#0e2016] border border-[#234b34] rounded-2xl max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden shadow-2xl animate-in fade-in duration-200">
        ...
        <div className="flex items-center justify-between p-4 border-b border-[#1b3c29]">
          <div className="flex items-center gap-2">
            <Film className="w-5 h-5 text-lime-400" />
        ...
        <div className="flex-1 overflow-auto p-4 flex flex-col items-center justify-center bg-[#070e0a] relative">
          <video
            src={videoUrl}
            controls
            autoPlay
            loop
            playsInline
            className="max-h-[55vh] max-w-full rounded-lg shadow-2xl object-contain bg-black"
          />

          {dialogue && (
            <div className="mt-3 max-w-lg w-full bg-[#0d1f15] border border-[#224732] p-2.5 rounded-lg text-center text-xs text-emerald-200 flex items-center justify-center gap-2">
        ...
        <div className="p-4 border-t border-[#1b3c29] bg-[#0c1a12] text-xs">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-lime-400 font-semibold">Prompt chuyển động video:</span>
            <button
              onClick={handleCopy}
